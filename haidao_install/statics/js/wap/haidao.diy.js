...
var Base = {
	...
	newJsFile: function(N, V){	//加载模块自定义JS
...
var diy = {
	init: function(config){
		...
		$.each(config, function(key) {
			...
			if(this.jsfile) Base.newJsFile(key, this.jsfile);
			if(this.cssfile) Base.newCssFile(key, this.cssfile);
		});
...
var Opts = {
	...
	goods: function(data,config){
		...
		function getCategoryText(cid){
			var a = [];
			$.each(config.libs.data, function(){
				if(this.id == cid){
					a.push(this.name);
