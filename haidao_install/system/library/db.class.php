...
class db {
    ...
    public function factory($db_config='') {
        ...
        if(empty($db_config['dbms']))
            throw new Exception(lang('_no_db_config_'));
        // 数据库类型
        $this->dbType = strtolower($db_config['dbms']);
        $class = 'db_'. $this->dbType;
        require_cache(LIB_PATH.'driver/db/'.$class.EXT);
        // 检查驱动类
        if(class_exists($class)) {
            ...
            // 类没有定义
            throw new Exception(lang('_no_db_driver_').': ' . $class);
        }
        return $db;
    }
    ...
    protected function multiConnect($master=false) {
        ...
        // 数据库读写是否分离
        if(config('DB_RW_SEPARATE')){
            // 主从式采用读写分离
            if($master)
                // 主服务器写入
                $r  =   floor(mt_rand(0,config('DB_MASTER_NUM')-1));
            else{
                if(is_numeric(config('DB_SLAVE_NO'))) {// 指定服务器读
                    $r = config('DB_SLAVE_NO');
                }else{
                    // 读操作连接从服务器
                    $r = floor(mt_rand(config('DB_MASTER_NUM'),count($_config['hostname'])-1));   // 每次随机连接的数据库
                }
            }
        }else{
            // 读写操作不区分服务器
    ...
    protected function parseSet($data) {
        foreach ($data as $key=>$val){
            if(is_array($val) && 'exp' == $val[0]){
              ...
              if(config('DB_BIND_PARAM') && 0 !== strpos($val,':')){
                $name   =   md5($key);
            ...
            foreach ($where as $key=>$val){
                ...
                if(0===strpos($key,'_')) {
                    ...
                    if(!preg_match('/^[A-Z_\|\&\-.a-z0-9\(\)\,]+$/',trim($key))){
                        throw new Exception(lang('_express_error_').':'.$key);
                    }
                    // 多条件支持
        ...
        if(is_array($val)) {
            if(is_string($val[0])) {
                if(preg_match('/^(EQ|NEQ|GT|EGT|LT|ELT)$/i',$val[0])) { // 比较运算
                    ...
                    throw new Exception(lang('_express_error_').':'.$val[0]);
                }
            }else {
                $count = count($val);
            ...
            //对字符串类型字段采用模糊匹配
            if(config('DB_LIKE_FIELDS') && preg_match('/('.config('DB_LIKE_FIELDS').')/i',$key)) {
                $val  =  '%'.$val.'%';
    ...
    protected function parseJoin($join) {
		...
		$joinStr = preg_replace("/__([A-Z_-]+)__/esU",config("DB_PREFIX").".strtolower('$1')",$joinStr);
        return $joinStr;
    }
    ...
    public function buildSelectSql($options=array()) {
        ...
        if(config('DB_SQL_BUILD_CACHE')) { // SQL创建缓存
        ...
        if(isset($key)) { // 写入SQL创建缓存
            cache($key,$sql,array('expire'=>0,'length'=>config('DB_SQL_BUILD_LENGTH'),'queue'=>config('DB_SQL_BUILD_QUEUE')));
        }
        return $sql;
    }
