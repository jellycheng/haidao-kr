...
class order_sub_service extends service {
	...
	public function get_subs($sn ,$skus = FALSE ,$track = FALSE ,$group = FALSE) {
		...
		foreach ($subs as $key => $sub) {
			...
			if ($skus == TRUE) {
                ...
                $goods_sku = $this->service_goods_sku->fetch_by_id($_skus[0]['sku_id']);
                if($goods_sku['edition'] == $_skus[0]['sku_edition']){
                    $_skus[0]['url'] = url('goods/index/detail',array('sku_id'=>$_skus[0]['sku_id']));
                }else{
                    $_skus[0]['url'] = url('goods/index/snapshot',array('sku_id'=>$_skus[0]['sku_id'],'order_sku_id'=>$_skus[0]['id']));
                }
	...
	public function set_order($sn = '',$action = '',$status = 0 ,$options = array()) {
		...
		if (empty($action)) {
			$this->error = lang('order/operate_type_empty');
			return FALSE;
		}
		if (!in_array($action, array('order','pay','confirm','delivery','finish'))) {
			$this->error = lang('order/operate_type_error');
			return FALSE;
		}
		// 检测订单是否存在
		...
		if (!$this->order) {
			$this->error = lang('order/order_not_exist');
			return FALSE;
		}
		// 获取订单状态
		...
		switch ($status) {
			case 2:	// 取消订单
				...
				if ($order['status'] != 1 || $order['delivery_status'] != 0) {
					$this->error = lang('order/order_dont_operate');
					return FALSE;
				}
			...
			case 4:
				...
				if (defined('IN_ADMIN')) {
					...
					if (!$result) {
						$this->error = lang('order/delete_order_error');
						return FALSE;
					}
					// 删除订单商品
