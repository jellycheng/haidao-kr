...
class database_control extends init_control {
	...
	public function optimize($tables = null){
		...
		if($tables) {
			...
			if(is_array($tables)){
				...
				if($list){
					showmessage(lang('admin/optimize_data_success'),'',1);
				} else {
					showmessage(lang('admin/optimize_data_error'),'',0);
				}
			} else {
				$list = $Db->query("OPTIMIZE TABLE `{$tables}`");
				if($list){
			...
			showmessage(lang('admin/optimize_data_empty'));
		}
	}
	...
	public function repair($tables = null){
		...
		if($tables) {
			...
			if(is_array($tables)){
				...
				if($list){
					showmessage(lang('admin/repair_data_success'));
				} else {
					showmessage(lang('admin/repair_data_error'));
				}
			} else {
				$list = $Db->query("REPAIR TABLE `{$tables}`");
				if($list){
			...
			showmessage(lang('admin/repair_data_empty'));
		}
	}
	...
	public function del($time = 0){
		...
		if($time){
			...
			if(count(glob($path))){
				showmessage(lang('admin/no_promission_delete'));
			} else {
				showmessage(lang('admin/delete_backups_success'),url('index',array('type'=>'import')),1);
			}
		} else {
			showmessage(lang('_param_error_'));
		}
	...
	public function export(){
		...
		if(!empty($tables) && is_array($tables)){ //初始化
			...
			//检查备份目录是否可写
			is_writeable($config['path']) || showmessage(lang('admin/backup_not_exist_success'));
			...
			if(false !== $database->create()){
				...
				showmessage(lang('admin/backup_set_error'));
			}
	...
	public function import(){
		...
		if(is_numeric($time) && (is_null($part)||empty($part)) && (is_null($start)||empty($start))){ //初始化
			...
			if(count($list) === $last[0]){
				...
				showmessage(lang('admin/init_ok'), '',1, array('part' => 1, 'start' => 0));
			} else {
				showmessage(lang('admin/backup_damage'));
			}
			...
			if(false === $start){
				showmessage(lang('admin/restore_data_error'));
			} elseif(0 === $start) { //下一卷
				if(isset($list[++$part])){
					...
					showmessage(lang('admin/restore_data_success'));
				}
			} else {
