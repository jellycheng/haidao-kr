...
class upgrade_control extends init_control
{
	...
	public function _initialize() {
		...
		if(FALSE === $this->cloud_config) showmessage(lang('admin/no_bind_site'),url('admin/cloud/index'));
		if(FALSE === $this->service->getcloudstatus()) showmessage(lang('admin/cloud_communication_error'),url('index'));
	...
	public function lists() {
		...
		$load->librarys('View')->assign('r',$r);
		$load->librarys('View')->display('upgrade_lists');
	}
	...
	public function upgrade() {
		...
		showmessage(lang('admin/start_upload'),url('upgrade_setp',array('setp'=>0)));
	...
	public function upgrade_setp(){
		...
		if (!array_key_exists($setp,$caches)){
			...
			showmessage(lang('admin/versions_upload_success'), url('index'), 1);
		}
		$this->downpack($caches[$setp]['version']);
		showmessage(lang('admin/versions_uploading').$caches[$setp]['version'].' ，请稍候...', url('upgrade_setp', array('setp' => $setp + 1)), 1);
	}
	...
	public function expfile($file, $version) {
		...
		if($md5file != $_version[$version]['md5']){
			...
			showmessage(lang('admin/download_again'),url('index'),0);
		}
		...
		if ($archive -> extract(PCLZIP_OPT_PATH, './', PCLZIP_OPT_REPLACE_NEWER) == 0) {
			exit(showmessage(lang('admin/upload_file_no_exist'),'null',0));
		} else {
