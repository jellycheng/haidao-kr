 ...
 * @return mixed
 */
function lang($name=null, $file = 'language') {
    static $_lang = array();
    $default_lang = config('DEFAULT_LANG');
    $path = LANG_PATH.config('DEFAULT_LANG').'/';
    //引入语言包
    ...
    if(is_string($name)) {
        if(strpos($name,'/') !== false){
            list($module, $indicate) = explode('/', $name);
            $path = APP_PATH.config('DEFAULT_H_LAYER').'/'.$module.'/language/'.$default_lang.'/';
        }elseif (strpos($name,'#') !== false) {
            list($pluginid, $indicate) = explode('#', $name);
            $path = APP_PATH.'plugin/'.$pluginid.'/language/'.$default_lang.'/';
        }else{
            return $_lang[$name] ? $_lang[$name] : $name;
        }
        $fullname = $path.$file.EXT;
        if(!isset($_lang[md5($fullname)])) {
            $_lang[md5($fullname)] = array();
            if(is_file($path.$file.'.php') && file_exists($path.$file.'.php')) $array = include $path.$file.'.php';
            $_lang[md5($fullname)] = array_merge($_lang[md5($fullname)], array_change_key_case($array, CASE_LOWER));  
        }
        return $_lang[md5($fullname)][$indicate] ? $_lang[md5($fullname)][$indicate] : $name;
    }
    return null;
}
...
function url($name, $param = '', $domain = FALSE) {
    ...
    if($name[0] == '#'){
        ...
        $data[config('VAR_CONTROL')] = 'plugin';
